...
- **`Task` ツールで別のサブエージェントを起動しない。** 他のスキルや commander も自分で起動しない。
  手は自分で動かす。（「さらに別のエージェントに再委任しない」という言い方だけでは
  「レビューのために `Task` ツールでエージェントを立てるのは別枠」と解釈されうる。実例:
  完了報告を書き終えたのに、レビュー用に起動した `Task` ツールのサブエージェント（計7体）が
  3日20時間 実行中のまま残っていた。禁止の対象は `Task` ツールそのものだと明記する）
...
## 検証の原則（デプロイ・配信・投稿を伴うタスクの場合）

**検証は利用者が見る最終成果物そのもので行う。中間生成物が正しいことは、最終成果物が
正しいことを意味しない。** 実例: タイル・属性・`index.json` は隅々まで検証して緑にしたのに、
実際にはスタイル定義（描画レイヤ）が抜けていて地図に何も表示されない、という事故が2回起きた。
検証対象に「利用者が実際に開くもの」を含めていなかったのが原因。

**間接的な兆候から進捗や成否を推測しない。一次情報を見る。** 実例:
「タイルのバイトサイズが一致しているから進んでいる」という間接的な兆候から
デプロイが進行中だと判断したが、実際は S3 に1ファイルもコピーされていなかった。
進捗・成否の判断は、対象システムを直接見て（S3 を直接 `ls` する、実際に配信 URL を叩く等）行う。

**「自分が編集したファイルを読み返す」のは検証ではない。** 実例: 「sprite を差し込み、
トグル動作確認済み」と3回にわたり報告したが、配信物（`index.html`）は1バイトも変わっておらず
mtime も変わっていなかった。原因は別ディレクトリのコピーを編集していたこと。
編集した場所ではなく**成果物が実際に届いている場所**を見る:

- Web に配信するものは `curl` で取得して確認する（例: `curl -sI <URL>` でサイズ・更新日時、
  `curl -s <URL> | grep <追加した文字列>` で中身の反映を確認）
- S3 に上げたものは `aws s3 ls` または公開 URL で確認する
- GitHub に出したものは `gh` で取得して確認する（`gh api` / `gh pr diff` 等）

**REPORT.md の「検証（コマンドと出力）」の節には、実際に叩いたコマンドとその生の出力をそのまま貼る。**
「確認しました」「反映されているはず」のような文章だけでは検証したことにならない。
...
## 検証（コマンドと出力）
<配信・投稿を伴うタスクは必須。自分が編集したファイルを読み返すのは検証ではない。
成果物が実際に届いている場所（Web配信ならcurl、S3ならaws s3 ls/公開URL、GitHubならgh）に対して
実際に叩いたコマンドと、その生の出力をそのまま貼る。配信・投稿を伴わないタスクは「対象外」と書く>
## テスト
